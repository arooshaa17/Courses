...
from pages.locators import page_selectors_list, footer_elements_list, course_card_href_list
...
class CourseTest(BaseTest):
    ...
    def test_course_cards_response(self):
        """
            Verify that images for all the course cards are loaded properly
        """
        self.course_page.explore_courses()
        result = self.course_page.verify_href_response_code(course_card_href_list)
        self.assertTrue(result, "Response code is not OK")
