BASE_URL = "https://www.edx.org/"
DASHBOARD = "https://courses.edx.org/dashboard"
DASHBOARD_TITLE = 'Dashboard | edX'
PROGRAMS_TITLE = 'Programs | edX'
DISCOVER_TITLE = 'edX | Search'
EXPECTED_HOME_PAGE_TITLE_LIST = ['Dashboard | edX', 'Programs | edX', 'edX | Search']

# Login
EMAIL = 'newtest5718@gmail.com'
PASSWORD = 'testselenium123'
INVALID_PASSWORD = 'abc123'

# Courses
COURSE_NAME = "Introduction to Python: Absolute Beginner"
COURSE_NAME2 = "Introduction to Analytics Modeling"
SEARCH_KEY = "python"
VERIFIED_LINK = "Verified"
PARTNER_VALUE = "HarvardX"
EXPECTED_FILTERS = ["python", "Verified", "HarvardX"]
IMAGE_PATH = "C:\\Users\\Aroosha Arif\\Desktop\\sunrise.jpg"


